...
//    private data class IsolatedClassLoaderCacheKey(val classpath: Set<File>)
    ...
    private fun runViaExec(args: List<String>) {
        KotlinLogger.warning("runViaExec:konanHome=$konanHome")
...
internal fun ClassLoader.runKonanTool(
    ...
    KotlinLogger.warning("runKonanTool1")
    ...
    KotlinLogger.warning("runKonanTool2")
    try {
        val mainClass = loadClass(mainClass)
        KotlinLogger.warning("runKonanTool3:mainClass=${mainClass}")
        KotlinLogger.warning("runKonanTool4:daemonEntryPoint=${daemonEntryPoint}")
        val entryPoint = mainClass.methods
                .singleOrNull { it.name == daemonEntryPoint } ?: error("Couldn't find daemon entry point '$daemonEntryPoint'")
        val s = KonanTarget.predefinedTargets.keys.joinToString(separator = ", ")
        KotlinLogger.warning("runKonanTool5,${s}")
        entryPoint.invoke(null, (listOf(toolName) + transformedArgs).toTypedArray())
    } catch (t: InvocationTargetException) {
        KotlinLogger.warning("runKonanTool6:t=${t.stackTrace}")
        throw t.targetException
    }
    KotlinLogger.warning("runKonanTool7")
}
