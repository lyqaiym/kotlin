...
import org.jetbrains.kotlin.gradle.plugin.mpp.KotlinJvmOhosCompilationFactory
...
import org.jetbrains.kotlin.gradle.plugin.sources.ohos.KotlinOhosSourceSets.applyKotlinOhosSourceSetLayout
...
internal class OhosProjectHandler(
    ...
    fun configureTarget(kotlinAndroidTarget: KotlinOhosTarget) {
        ...
        project.forAllOhosVariants { variant ->
            val compilationFactory = KotlinJvmOhosCompilationFactory(kotlinAndroidTarget, variant)
            val variantName = getVariantNameOhos(variant)
            ...
            compilationFactory.create(variantName).let { compilation ->
                ...
                (kotlinAndroidTarget.compilations as NamedDomainObjectCollection<in KotlinJvmOhosCompilation>).add(compilation)
            }
        ...
        project.whenEvaluated {
            forAllAndroidVariants { variant ->
                val compilation = kotlinAndroidTarget.compilations.getByName(getVariantNameOhos(variant))
    ...
    private fun addKotlinDependenciesToAndroidSourceSets(project: Project) {
         ...
         * see [org.jetbrains.kotlin.gradle.plugin.OhosProjectHandler.configureTarget]
         */
    ...
    private fun preprocessVariant(
        ...
        compilation: KotlinJvmOhosCompilation,
        project: Project,
        ...
        val variantDataName = getVariantNameOhos(variantData)
        ...
        compilation.androidVariant?.forEachKotlinSourceSetOhos(project) { kotlinSourceSet ->
            @Suppress("DEPRECATION_ERROR")
    ...
    private fun postprocessVariant(
        ...
        getTestedVariantDataOhos(variantData)?.let { testedVariant ->
            val testedVariantName = getVariantNameOhos(testedVariant)
    ...
    private fun wireKotlinTasks(
        ...
        val preJavaKotlinOutput = project.files(project.provider {
            mutableListOf<File>().apply {
                ...
                if (Kapt3GradleSubplugin.isEnabled(project)) {
                    ...
                    val kaptClassesDir = project.getKaptGeneratedClassesDirectory(getVariantNameOhos(variantData))
        ...
        val testedVariantDataIsNotNull = getTestedVariantDataOhos(variantData) != null
...
@Suppress("TYPEALIAS_EXPANSION_DEPRECATION")
internal fun getTestedVariantDataOhos(
    variantData: DeprecatedOhosBaseVariant
): DeprecatedOhosBaseVariant? = when (variantData) {
    is DeprecatedOhosTestVariant -> variantData.testedVariant
    is DeprecatedOhosUnitTestVariant -> variantData.testedVariant as? DeprecatedOhosBaseVariant
    else -> null
}

internal fun getVariantNameOhos(
    @Suppress("TYPEALIAS_EXPANSION_DEPRECATION") variant: DeprecatedOhosBaseVariant
): String = variant.name

/** Filter for the AGP test variant classpath artifacts. */
class OhosTestedVariantArtifactsFilter(
...
@Suppress("TYPEALIAS_EXPANSION_DEPRECATION")
internal inline fun DeprecatedOhosBaseVariant.forEachKotlinSourceSetOhos(
...
@Suppress("TYPEALIAS_EXPANSION_DEPRECATION")
internal inline fun DeprecatedOhosBaseVariant.forEachKotlinSourceDirectorySetOhos(
...
@Suppress("TYPEALIAS_EXPANSION_DEPRECATION")
internal inline fun DeprecatedOhosBaseVariant.forEachJavaSourceDirOhos(
