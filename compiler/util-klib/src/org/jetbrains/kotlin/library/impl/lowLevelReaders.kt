...
fun IrArrayReader(bytes: ByteArray): IrArrayReaderNew = IrArrayReaderNew(ReadByteBufferProvider.MemoryBuffer(bytes))

/** On-demand read from a byte array that will be loaded on the first access. */
fun IrArrayReader(loadBytes: () -> ByteArray): IrArrayReaderNew = IrArrayReaderNew(ReadByteBufferProvider.OnDemandMemoryBuffer(loadBytes))
...
): IrArrayReaderNew = IrArrayReader { layoutReader.readInPlace { it.getFile().readBytes() } }

class IrArrayReaderNew(private val buffer: ReadByteBufferProvider) {
...
/** Read directly from a byte array. */
fun IrMultiArrayReader(bytes: ByteArray): IrMultiArrayReaderNew = IrMultiArrayReaderNew(ReadByteBufferProvider.MemoryBuffer(bytes))

/** On-demand read from a byte array that will be loaded on the first access. */
fun IrMultiArrayReader(loadBytes: () -> ByteArray): IrMultiArrayReaderNew = IrMultiArrayReaderNew(ReadByteBufferProvider.OnDemandMemoryBuffer(loadBytes))

/** On-demand read from a file (potentially inside a KLIB archive file). */
inline fun <KCL : KlibComponentLayout> IrMultiArrayReader(
    layoutReader: KlibLayoutReader<KCL>,
    crossinline getFile: KCL.() -> File,
): IrMultiArrayReaderNew = IrMultiArrayReader { layoutReader.readInPlace { it.getFile().readBytes() } }

class IrMultiArrayReaderNew(private val buffer: ReadByteBufferProvider) {
    private val indexToOffset: IndexToOffset = buffer.use { it.readIndexToOffset(0) }
...
data class DeclarationIdNew(val id: Int)
...
class DeclarationIdTableReader(private val buffer: ReadByteBufferProvider) {
    ...
    fun tableItemBytes(declarationId: DeclarationIdNew): ByteArray =
...
class DeclarationIdMultiTableReader(private val buffer: ReadByteBufferProvider) {
    ...
    fun tableItemBytes(index: Int): ByteArray = buffer.use { it.readTableItemBytes(indexToOffset, index) }
    fun tableItemBytes(rowIndex: Int, declarationId: DeclarationIdNew): ByteArray =
...
fun IrArrayReaderNew.toArray(): Array<ByteArray> = Array(this.entryCount()) { i -> this.tableItemBytes(i) }
...
private typealias DeclarationIdToCoordinates = MutableMap<DeclarationIdNew, DeclarationCoordinates>
...
private fun ByteBuffer.readDeclarationIdToCoordinates(position: Int): DeclarationIdToCoordinates {
    ...
    for (i in 0 until count) {
        val declarationId = DeclarationIdNew(this.int)
        val offset = this.int
        val size = this.int
...
private fun ByteBuffer.readTableItemBytes(declarationIdToCoordinates: DeclarationIdToCoordinates, declarationId: DeclarationIdNew): ByteArray {
